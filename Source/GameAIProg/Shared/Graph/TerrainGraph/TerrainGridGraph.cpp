...
std::unordered_map<TerrainNode::Type, float> const TerrainGridGraph::TerrainCostMultipliers
{
	...
	{TerrainNode::Type::Water, FLT_MAX},  
...
void TerrainGridGraph::PaintNodeAtPosition(FVector2D const& Position, TerrainNode::Type TypeToPaint)
{
	...
	if (TypeToPaint == TerrainNode::Type::Water)
	{
		...
		int row = NodeId / NrColumns;
		int col = NodeId % NrColumns;

		int left = GetNodeId(col - 1, row);
		int right = GetNodeId(col + 1, row);

		if (left != -1 && right != -1)
		{
			RemoveConnection(left, right);
			RemoveConnection(right, left);
		}

		int up = GetNodeId(col, row - 1);
		int down = GetNodeId(col, row + 1);

		if (up != -1 && down != -1)
		{
			RemoveConnection(up, down);
			RemoveConnection(down, up);
		}
	...
	auto ConnectionsToUpdate = FindConnectionsWith(NodeId);
	for (Connection* Conn : ConnectionsToUpdate)
	{
		TerrainNode* FromNode = GetNodeAs<TerrainNode>(Conn->GetFromId());
		TerrainNode* ToNode = GetNodeAs<TerrainNode>(Conn->GetToId());

		float FromMult = *GetTerrainCostMultiplier(FromNode->GetType());
		float ToMult = *GetTerrainCostMultiplier(ToNode->GetType());

		float BaseCost = IsCardinalConnection(Conn->GetFromId(), Conn->GetToId())
			? GetCardinalCost()
			: GetDiagonalCost();

		float FinalCost = BaseCost * 0.5f * (FromMult + ToMult);
		Conn->SetWeight(FinalCost);
	}
}
